...
module rv32i_soc #(parameter CLK_FREQ_MHZ=12, PC_RESET=32'h00_00_00_00, TRAP_ADDRESS=32'h00_00_00_00, ZICSR_EXTENSION=1, MEMORY_DEPTH=81920, GPIO_COUNT = 12, BAUD_RATE = 9600, MEM_INIT_FILE = "memory.mem", ENABLE_AXI = 1) ( 
    input wire i_clk,
    input wire i_rst,
    //UART
    input wire uart_rx,
    ...
    wire device6_wb_stb;
    wire device6_wb_we;
    wire[31:0] device6_wb_addr;
    wire[31:0] o_device6_wb_data;
    wire[3:0] device6_wb_sel;
    wire device6_wb_ack;
    wire device6_wb_stall;
    wire[31:0] i_device6_wb_data;
    ...
    memory_wrapper wrapper( //decodes address and access the corresponding memory-mapped device
        ...
        //Device 6 Interface (AXI Master)
        .o_device6_wb_cyc(device6_wb_cyc),
        .o_device6_wb_stb(device6_wb_stb),
        .o_device6_wb_we(device6_wb_we),
        .o_device6_wb_addr(device6_wb_addr),
        .o_device6_wb_data(o_device6_wb_data),
        .o_device6_wb_sel(device6_wb_sel),
        .i_device6_wb_ack(device6_wb_ack),
        .i_device6_wb_stall(device6_wb_stall),
        .i_device6_wb_data(i_device6_wb_data)
    );   

    // DEVICE 0
     main_memory #(.MEMORY_DEPTH(MEMORY_DEPTH), .MEM_INIT_FILE(MEM_INIT_FILE)) m1( //Instruction and Data memory (combined memory) 
        .i_clk(i_clk),
        // Instruction Memory
    ...
    // DEVICE 2
    uart #( .CLOCK_FREQ(CLK_FREQ_MHZ*1_000_000), //UART [memory-mapped to >=h50,<hA0 (MSB=1)]
            .BAUD_RATE(BAUD_RATE), //UART Baud rate (configurable via parameter)
    ...
    // DEVICE 6: AXI-Lite Master Bridge
    generate if (ENABLE_AXI == 1) begin : axi_master_gen
        
        // Wishbone to AXI User Control translation
        wire user_wr_req  = device6_wb_cyc && device6_wb_stb && device6_wb_we;
        wire user_rd_req  = device6_wb_cyc && device6_wb_stb && !device6_wb_we;
        wire user_wr_done, user_rd_done;
        
        // Acknowledge logic: ack is asserted by the AXI master when done
        assign device6_wb_ack   = user_wr_done || user_rd_done;
        // Stall logic: stall the CPU pipeline while the AXI transaction is in progress
        assign device6_wb_stall = (user_wr_req && !user_wr_done) || (user_rd_req && !user_rd_done);
        ...
        axi_lite_master #(
            ...
            // User Interface connected to Wishbone logic
            .user_wr_req  (user_wr_req),
            .user_wr_addr (device6_wb_addr),
            .user_wr_data (o_device6_wb_data),
            .user_wr_done (user_wr_done),
            .user_wr_resp (), 
            
            .user_rd_req  (user_rd_req),
            .user_rd_addr (device6_wb_addr),
            .user_rd_data (i_device6_wb_data),
            .user_rd_done (user_rd_done),
            .user_rd_resp ()  
        );
    end else begin : axi_master_gen
        // Tie-offs if AXI is disabled
        ...
        assign device6_wb_ack      = 0;
        assign device6_wb_stall    = 0;
        assign i_device6_wb_data    = 0;
    end endgenerate
...
module memory_wrapper ( //decodes address and access the corresponding memory-mapped device
    ...
    //Device 6 Interface (AXI)
    output reg o_device6_wb_cyc,
    output reg o_device6_wb_stb,
    output reg o_device6_wb_we,
    output reg[31:0] o_device6_wb_addr,
    output reg[31:0] o_device6_wb_data,
    output reg[3:0] o_device6_wb_sel,
    input wire i_device6_wb_ack,
    input wire i_device6_wb_stall,
    input wire[31:0] i_device6_wb_data
);
    ...
    always @* begin 
        ...
        if(i_wb_addr[31]) begin
            ...
            if(i_wb_addr[11:0] >= 12'hF0 && i_wb_addr[11:0] < 12'h100) begin //Device 4 Interface (GPIO) (20 words)
            ...
            if(i_wb_addr[31:28] == 4'hC) begin //Device 5 Interface (DDR3)
            ...
            if(i_wb_addr[11:0] >= 12'h100) begin //Device 6 Interface (AXI Master)
                o_device6_wb_cyc = i_wb_cyc;
                o_device6_wb_stb = i_wb_stb;
                o_device6_wb_we = i_wb_we;
                o_device6_wb_addr = i_wb_addr; 
                o_device6_wb_data = i_wb_data;
                o_device6_wb_sel = i_wb_sel; 
                o_wb_ack = i_device6_wb_ack;
                o_wb_stall = i_device6_wb_stall;
                o_wb_data = i_device6_wb_data;
            end
        end
...
module main_memory #(parameter MEMORY_DEPTH=1024, parameter MEM_INIT_FILE = "memory.mem") ( //Instruction and Data memory (combined memory)
    input wire i_clk,
    // Instruction Memory
    ...
    initial begin //initialize memory
        ...
        o_wb_ack <= 0;
        o_inst_out <= 0;
        $readmemh(MEM_INIT_FILE, memory_regfile); //load firmware into memory (path configurable via MEM_INIT_FILE parameter)
    end
